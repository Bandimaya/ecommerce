...
export default function WhyStempark() {
  const [activeId, setActiveId] = useState<string>(stemparkFeatures[0].id);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setActiveId((currentId) => {
        const currentIndex = stemparkFeatures.findIndex(f => f.id === currentId);
        const nextIndex = (currentIndex + 1) % stemparkFeatures.length;
        return stemparkFeatures[nextIndex].id;
      });
    }, AUTOPLAY_INTERVAL);
          ...
          {stemparkFeatures.map((feature) => {
            ...
            return (
              <motion.div
                ...
                onMouseEnter={() => window.innerWidth >= 1024 && setActiveId(feature.id)}
                className={cn(
