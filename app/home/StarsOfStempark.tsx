...
export default function StarsOfAvishkaar() {
  ...
  const activeStar = starsData[currentIndex];
  ...
  const handleNext = () => {
    setDirection(1);
    setCurrentIndex((prev) => (prev + 1) % starsData.length);
  };

  const handlePrev = () => {
    setDirection(-1);
    setCurrentIndex((prev) => (prev - 1 < 0 ? starsData.length - 1 : prev - 1));
  };
        ...
        <div className="flex items-center justify-between w-full">
            <div className="flex flex-col">
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">
                    Student {currentIndex + 1} of {starsData.length}
                </span>
                <div className="h-1 w-24 bg-muted mt-2 rounded-full overflow-hidden">
                        <motion.div 
                        className="h-full bg-primary"
                        initial={{ width: 0 }}
                        animate={{ width: `${((currentIndex + 1) / starsData.length) * 100}%` }}
                        />
                </div>
            </div>
